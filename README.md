...
| `076_somebody_has_to_be_able_to_read_an_application` | **The admissions form had been live and nobody at the masjid could open what it wrote.** 008 built the whole pipeline and stopped one function short: a parent applies, the office is emailed, the row is purged three years later, and in between there is no list, no record and no decision — `admission_applications` is FORCE RLS with no grant, so the table is sealed. It had never shown up because nobody had applied yet. Adds the overview, the list, the record and the decision. The **list carries flags and never detail** — a check fails if it ever learns `send_detail`, `allergy_detail` or `medical_conditions` — and **opening one child's record writes an audit row**, because "who has read this" is a question a subject access request asks. Also revokes the direct `SELECT` 008 gave `authenticated`: the policy behind it required `verified_admin()` so nothing was ever exposed, but an administrator could read every application through PostgREST without ever calling the function that writes down who looked, and an audit trail with a documented bypass is worse than none. |
| `079_status_export_and_a_guard_that_runs` | **A pupil can be on hold, an administrator can take a file out, and the rule that stops a list learning a child's medical note now actually runs.** Three things. **Status**: the madrasah could record two states, left or not, so a child who had stopped for a term had nowhere to be except *on roll*. Four states, with a constraint tying `left` to a leaving date so the two can never disagree, and a check that tries three wrong combinations before trusting it. The old system's eleven statuses are **not** copied - four of them are *application* states and Admissions owns those here, so nine would sit permanently at zero and a filter that is always empty teaches people to stop reading the filters. **Export**: the highest-risk feature in the portal, because everything else keeps children inside a screen and this puts 552 of them in a file that gets emailed. Two levels - a four-column register list for any staff member, a thirteen-column list for administrators - **and an audit row every time** naming who, when, which columns, how many rows and the filter in force, so *exported the whole roll* and *exported one class of twelve* are different entries. Neither carries medical, allergies or SEND. Proved end to end against the live register in a rolled-back transaction. **The guard**: 077 asserted that `madrasah_roll()` does not return a detail column and the note afterwards claimed the split *"cannot be undone by somebody adding just one field"*. **That was false** - the assertion ran once, at migration time, in a block long since finished; nothing re-ran it. It now lives in `health_check()`. Its first version **failed correct code**, flagging the four legitimate `p.medical is not null` marks, so the rule became *every mention must be immediately followed by IS NULL* - test it freely, return it and this fails. And the check that proved the guard originally did so by replacing the live export with a stub returning everyone's medical notes and relying on rollback; it now points at its own throwaway. Reading health for the first time also revealed the four `import_` landing tables had been failing it since this morning - exempted from the tenancy check and given one of their own that says out loud they are still holding a second copy of the register. |
| `078_what_the_register_got_wrong` | **The numbers reconciled and the register was still wrong about nine children.** 077 proved 552 pupils, 554 class places and 330 families all tallied; three sanity checks afterwards asked the harder question. Two spaces did the first damage: the register spells three class names with a double space where the September load used one, so the import did not recognise them and made a SECOND copy of each - the roll went to the new copy and the old sat empty beside it. Nothing was lost and the counts stayed right, which is precisely why it survived 077; a staff list showing *Boys Hafiz Class - C* twice, one of them empty, is how somebody marks a register against the wrong one. Merged, spacing normalised so the next import matches, and the four classes the register never mentions made inactive rather than deleted. Then nine dates of birth that cannot be right: a 51 year old in Boys Year 6, three 3 year olds in Ladies Naazirah, a four year old in Boys Year 8. **The obvious detectors are all wrong and each looked right until it was run** - "far from the class median" fires 41 times, 37 of them adult learners where a 21-to-64 spread is correct; "anyone under five" fires 55 times, 53 of them on Play and Pray and Reception, which EXIST for three and four year olds; "outside the class's own range" cannot catch three 3 year olds who together widen the range being measured. Three plain tests in three directions do it, and a check fails if the rule ever flags a correct infant enrolment. Nothing corrects a date - only a parent can settle one - the screen finds them. Also: **a pupil's own e-mail now counts as a contact**, because three adult students in the Ladies classes have no guardian (they ARE the adult) and the roll was calling them unreachable while holding their address. "No way to ring" fell from 10 to 3. |
| `077_the_register_552_pupils` | **The madrasah's real roll: 552 children, 330 families, 44 classes, 37 teachers.** The pupil record held a first name, a last name, two dates, a family and a fee rate, so there was nowhere for a date of birth, an address, a school, a medical note or an allergy — which is most of what the register holds. Fifteen columns, a unique pupil reference so the import is repeatable, and the same split Applications uses: `madrasah_roll()` returns marks and never detail (a check reads the function's own source and fails if it learns a detail column), while `madrasah_pupil_one()` returns the detail and **writes down who read it** — deliberately not `stable`, because the planner may elide a stable call and the audit row would vanish. **Ethnicity is deliberately absent**: blank in the sound export, unreadable in the damaged one, and Article 9 data besides — a column sitting empty is an invitation to fill it. An earlier load on 18 September had put 543 pupils in with no reference number, which the import could not have matched: **every child would have appeared twice**. So the import clears what it cannot match and then checks its own result, raising — and rolling back — if the numbers disagree. The first version of that guard compared the file against the madrasah *after* clearing, which can never fail; its own check caught it. The register's 552 children are uploaded through the Supabase dashboard and never pass through the assistant. |
| `075_the_reconciler_could_never_have_run` | **It threw every time it had work to do.** 072's reconciler wrote an audit row with `masjid_id` null into a NOT NULL column, after the loop — so the first reminder it resolved raised 23502 and rolled back every outcome it had just written. Reminders would have sat at `queued` for ever, holding the seven-day lock, and the six-hour rule that turns silence into an honest failure is inside the same aborted loop. Green every five minutes since 20 September purely because nothing was ever queued for it. Its check now RUNS the function against all three answers a mail server can give. |
| `074_a_200_is_not_a_message` | **notify answers 200 for a kind it does not know, on purpose**, so Supabase does not retry — and 072 read only the status code. A reminder that reached the mail server and produced no message would have been written down as sent. Edge functions deploy separately from migrations, so that window exists every time a new message kind is added; the fix is in the reconciler, which is the only place that can be taught the general rule. |
| `073_the_fees_functions_are_shut_to_anon` | **Every fees function was callable by a signed-out stranger.** Postgres grants EXECUTE to PUBLIC by default; forty earlier files here revoke it and 068–071 wrote only the grant. Nothing leaked — all of them call `verified_admin()` first — but the second lock was missing while every madrasah function written before them was properly shut. Found by Supabase's advisor after a test had reported the gate held: that test counted the exception from `verified_admin()` as though it were a permission denial, and both carry SQLSTATE 42501. |
...
| `064_reading_restoring_and_emptying_the_archive` | **Look, put back, delete for good.** Restore only re-links what still exists — a class archived after the teacher who taught it would fail the foreign key and take the whole restore down with it. `delete_archived_record()` is the one that answers an erasure request, and its audit row carries the kind and the date and **not the name**: recording the name there would leave behind the very thing somebody asked to have erased. `purge_madrasah_archive()` empties it three years on. |
| `063_nothing_is_deleted_it_is_archived` | **Removing something from the madrasah puts it in the archive.** Staff, pupils and classes. The whole row *and its links* go in as jsonb, so a class comes back with its register and a teacher with their timetable rather than an approximation. **An archive is still a record** — “archived” is not “deleted” to a regulator, which is why 064 adds a separate delete-for-good; without it “archive everything” quietly becomes “we never delete anything”. Same three-year clock as a pupil record. Proved by a round trip: class out, **child not deleted**, class and register both back, archive row gone so it cannot be restored twice. |
| `062_what_the_madrasah_holds_about_a_teacher` | **The Staff screen stops saying “Nothing on file” about people it has a file on.** From the masjid's own teacher sheet: 20 real DBS issue dates, addresses, dates of birth, telephone numbers and working hours. The DBS picture goes from 40 nothing-on-file to **19 valid, 1 overdue, 20 nothing on file**. **Four fields on that sheet are deliberately not here**, decided with the masjid: the DBS certificate number (052 already ruled on it — it does not prove a check happened, and is only useful to somebody impersonating the teacher), the National Insurance number (a lifelong identifier, the most useful field there is for identity fraud, and 3 of 40 populated), bank details, and gender. A check in the migration fails if any of them ever appears. Also caught on the way in: the PDF's two columns collapse into single lines, so a naive capture stored the word “Start” from the next column as a telephone number — every number is now validated for 10–13 digits before it is written. |
| `061_moving_a_child_in_and_out_of_a_class` | **Add and remove children, inside “Amend this class”.** Behind the same door as renaming the class, because taking a child off a register is a change to what the madrasah holds and should not be one press from simply reading it. An empty search returns nothing rather than everybody; each result shows the classes that child is already in, because twelve pairs of children in this madrasah share a name; and removing returns how many classes they have **left**, so the screen can say in words when it was their last one — a record in no class is reachable from no register. |
| `060_a_class_has_one_main_teacher` | **“Why does it show multiple teachers per class? That's wrong.” It was.** The class list PDF names the whole Apa team against the girls' progression classes — Girls OOLA carries six — and 052's import wrote every one into `madrasah_staff_classes`, so the card printed all six. A card answers *whose class is this*, and six names answer nothing. The system being replaced gets this right with a single Main Teacher field and this project missed it. 35 of 45 set automatically where exactly one member of staff was assigned; **10 left blank on purpose** — picking one of six Apas here would be inventing an answer the masjid has not given. |
| `059_bmcc_certificate` | **A charity cannot request a collection without showing its BMCC certificate.** Bolton Masjid Chanda Committee clears a charity to collect in Bolton's masjids; the masjid asked to see that certificate before approving, and it must be no more than three months old. That used to happen on paper, after the form — it is now part of the form. **The `bmcc` bucket is private, unlike `notices`.** 042 created a public bucket and said plainly that anything in it is on the internet to anybody with the address, which was right for posters and is wrong for this: a certificate carries an organisation's name, its registration, a committee signature and a date, handed over to satisfy one masjid rather than to be published. `anon` may INSERT into that one bucket and do nothing else — no select, no list, no update, no delete; it cannot read back even the file it just wrote. **The honest risk, stated in the migration:** anybody who finds the anon key (it is in the page source of every page, by design) can push files into the bucket until somebody notices. What they cannot do is read anything out of it. |
| `058_the_madrasah_has_pupils` | **The table this project refused to create for a week.** 052's own header says why it did not build it: *"A madrasah roll reveals a child's religion, which is Article 9 data before anybody adds a note ... So this could be built and the pupil tables could not."* It is built because the masjid has confirmed the DPIA is complete and the charity is ICO-registered. **"The site is not live yet" was offered as the reason and is not one** — the UK GDPR bites at PROCESSING, and writing five hundred children into a database in eu-west-2 is processing whether or not a parent can see it. Two decisions from the masjid are built in. **Who may see a child:** administrators see everyone; a madrasah account sees only the children in classes it is assigned to, joined through `auth.uid()` in Postgres — 40 teachers against 543 children is 21,720 possible pairings, and scoping to real assignments cuts it to about 543, which is the whole of the difference if an account is ever taken. **How long a record is kept:** three years after the child leaves, then deleted by `purge_madrasah_pupils()` — a delete, not a reminder. A safeguarding record is on a far longer clock and there is a check in the migration that fails if anything ever cascade-deletes from this table, because that would silently turn the three-year purge into a safeguarding purge. The table holds a name, a class and two dates. **No date of birth, address, parent's number, medical note or photograph** — the import has none and an empty column is an invitation. Also adds `delete_madrasah_class()`, which 052 never wrote. |
| `057_reading_and_writing_the_calendar_the_profile_and_the_brand` | **The functions for the calendar, the profile and the brand images, and the `brand` bucket.** Two of the fifteen are executable by **anon** and that is the whole design: `madrasah_calendar()`, because the public Holiday Planner is going to read the madrasah's dates from here instead of from an array typed into `index_template.html`; and `masjid_brand()`, because an email banner has to be fetchable by a mail client that is signed in to nothing. Everything else asks `verified_admin()`. `madrasah_people()` answers *who can open this portal and why* and is **read only with no matching writer anywhere in this file** — granting stays on one screen, because two screens that both grant access is how somebody is removed from one and quietly left on the other. `delete_masjid_image()` refuses to delete the one in use: every letter already sent points at it. |
| `056_the_madrasah_year_the_calendar_and_the_masjids_own_brand` | **The madrasah's year, its closures, its Islamic dates, and the masjid's own particulars and images.** Seeded with the **eight closures and ten Islamic dates copied exactly from the live Holiday Planner** — if a date here differs from the website, that is a bug in the migration and not a correction. Two kinds of date and they are not equally certain: a closure is the madrasah's own decision and a fact; an Islamic date is calculated and settled by moon sighting, so `is_estimated` **defaults to true** and only an explicit answer from the screen turns it off. Wrongly calling a confirmed date an estimate costs nothing; presenting a calculation as settled is how a family turns up for Eid on the wrong morning. A closure longer than 366 days is refused, because that is somebody typing 2037 for 2027 and it would paint the whole calendar shut without complaint. One current year and one current image per kind, enforced by partial unique indexes rather than by hope. |
| `055_the_madrasah_is_a_role` | **One line in a file of its own, and that is not an accident.** `user_roles.role` is an enum, and Postgres will not let a value be added and used in the same transaction — so the value lands here and everything that reads it is in 056. Written the other way round it would apply cleanly on a database where somebody had already added the value by hand and fail on a fresh one, which is the worst kind of migration. **Why a role at all:** access to the madrasah was *are you an administrator*, and an administrator can do everything the masjid does — hall bookings, donations, Gift Aid, a notification to every phone in the congregation. Handing the madrasah lead all of that to get at the register is how a system ends up with four administrators who are administrators because it was the only box to tick. The role reaches the **teaching side only**; staff, DBS, fees and admissions still ask `verified_admin()`. |
| `054_the_staff_screen_can_actually_write` | **Nobody had ever been able to save anybody on the staff screen, and this project broke it.** Every table in this database carries `masjid_id not null` with no default and every write function fills it from `current_masjid()`. 053 rebuilt `save_madrasah_staff()` and `madrasah_staff_list()` from 052's text, which predates those columns, and silently dropped the masjid out of both — so the insert failed with a not-null violation on **amendments as well as additions**, because the constraint is checked before `ON CONFLICT` looks for a conflict. Exactly the two functions 053 rewrote are exactly the two that lost their scoping; `save_madrasah_class()`, untouched, kept it. **The cause of the silence is that 053's file says “see the migration history for the full text” instead of carrying the functions, so no diff could show a column going missing. A migration that rebuilds a function prints the function.** Also moves what the previous system recorded about DBS out of the free-text `note` field, which belongs to whoever is using this system, into `prior_dbs` — the masjid asked for the old vendor's name to come off the records, and deleting the notes outright would have thrown away the only thing separating the twenty-two waiting on a certificate date from the sixteen with nothing on file, who are the actual safeguarding question. The column clears itself the moment a real date is entered. |
| `053_the_madrasah_teaches_two_sides` | **The staff list shows a sisters' side and a brothers' side, and which one a person is on could not be worked out.** Not from the honorific: Aisha Omarji is the one imported name with none, and a new member of staff need not have one. Not from their classes either: Apa Shamim Nasir teaches Boys Reception, which is ordinary — the youngest boys are taught by women — so deriving it would have put her name in the brothers' column of a safeguarding screen. So it is stored, seeded from the honorific only where that settles it, and left null where it does not. 23 sisters, 16 brothers, 1 not set. The screen shows the unset in their own labelled group: two columns that between them show 39 of 40 people is the failure this exists to avoid, because nobody notices the missing one. |
...
- **The public Holiday Planner still reads a hard-coded array.** The calendar
  ...
  it yet, so a date amended in the portal does not reach the website. Anybody
  amending one now has to amend it twice.
