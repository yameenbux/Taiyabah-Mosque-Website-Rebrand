-- ===========================================================================
--  024_admin_dashboard.sql — everything the admin centre shows, in one call
--
--  12 September 2026. /portals/ stops being a list of links and becomes the
--  page an administrator lands on: what needs doing, what the masjid looks
--  like, where they can go, and what has happened.
--
--  WHY ONE FUNCTION AND NOT EIGHT QUERIES. The page would otherwise open
--  eight round trips on load, each one separately subject to RLS, each one a
--  separate way to half-fail — a dashboard showing five panels and one
--  silent blank is worse than a dashboard that says it could not load. One
--  call, one answer, one error path.
--
--  AND WHY IT IS security definer. The counts cross tables whose policies
--  differ (hall_bookings is office-or-admin, donations is admin only). Doing
--  it in the browser means the browser deciding what to ask for, which is
--  the browser deciding what it is allowed to see. Here the ROLE CHECK IS AT
--  THE TOP and each section is gated on it, so what comes back is decided in
--  one place that the browser cannot argue with.
--
--  Prerequisites: 011, 022, 023. Idempotent.
--
...
begin
  ...
  if to_regclass('public.foodbank_volunteers') is null then
    raise exception 'public.foodbank_volunteers does not exist. Run 023_foodbank_volunteers.sql first.';
  end if;
end $$;


-- ---------------------------------------------------------------------------
--  1. Which audit actions are the machine's, and which are people's
--
--  THIS CLASSIFICATION IS THE WHOLE REASON THE LOG IS READABLE. On the day
--  this was written the audit trail held 130 rows, and 120 of them were one
--  automatic job clearing expired hall holds. A feed that showed them all
--  would be 92% machine, and nobody reads a log twice after the first time it
--  wastes their attention.
--
--  So: 'auto' is anything the masjid did not do — purges, scheduled digests,
--  repairs. 'public' is somebody on the website sending something in.
--  Everything else is a person in the office, and those are the rows that
--  carry a name.
-- ---------------------------------------------------------------------------
...
comment on function public.audit_kind(text) is
  'auto = the system did it, public = somebody on the website, staff = somebody in the office.';


-- ---------------------------------------------------------------------------
--  2. Plain English for the log
--
--  The office reads this, not a developer. 'volunteer_status' is not a
--  sentence.
-- ---------------------------------------------------------------------------
...
comment on function public.admin_dashboard() is
  'Everything /portals/ shows, in one call. Role-gated at the top: office sees halls, nikah and volunteers; admin sees all of it.';
...
--
--  Then check it answers. Signed out this must return {"allowed": false}:
--
--    select public.admin_dashboard();
--
--  And as a verified administrator it must NOT be null anywhere it matters:
--
