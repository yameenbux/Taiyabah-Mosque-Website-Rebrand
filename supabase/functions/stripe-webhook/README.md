...
Records a paid Astley Hall deposit. Nothing else in the system is allowed to
say that money arrived.

## Setting it up, in order

You do not need the Supabase CLI. Everything below can be done in the browser.

1. **Create the function in the dashboard.**

   Edge Functions → **Deploy a new function** → **Via Editor**. Name it
   `stripe-webhook`, replace the template with `index.ts` from this folder, and
   press Deploy. It takes half a minute.

2. **Turn OFF "Verify JWT with legacy secret"** on the function's settings.

   This is not optional and it is the single likeliest thing to go wrong.
   Stripe does not send a Supabase JWT, so with verification on, every delivery
   is rejected with a 401 *before this file ever runs* — no logs, nothing in
   `admin_audit`, and the function looks broken when it is fine.

   **Check it again after every redeploy.** The toggle is known to switch
   itself back on when a function is updated, from the dashboard and from the
   CLI alike. If deliveries suddenly start failing after you edit the function,
   look here first.

   *(CLI equivalent, if you would rather:
   `supabase functions deploy stripe-webhook --no-verify-jwt`.)*

3. **Add the endpoint in Stripe** — Developers → Webhooks → Add endpoint.
   URL: `https://<project-ref>.supabase.co/functions/v1/stripe-webhook`
   Subscribe it to **`checkout.session.completed`** and nothing else.

4. **Set one secret.** Project Settings → Edge Functions → Secrets.

   | Name | Value |
   |---|---|
   | `STRIPE_WEBHOOK_SECRET` | `whsec_…`, shown when you created the endpoint in step 3 |

   **Do not add your Stripe API key.** Verifying a webhook signature is a local
   HMAC against the *webhook* secret — it makes no call to Stripe and never
   touches an API key. Putting `sk_live_…` here would store the masjid's most
   dangerous credential, one that can issue refunds and read every customer, in
   a system with no use for it.

   `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are provided by the platform.
   Do not add them by hand.

   The webhook secret only exists once the endpoint does, so this has to come
   after step 3.

5. **Point the Payment Link back at the website.** In Stripe, edit the £100
   deposit link → After payment → redirect to
   `https://www.taiyabahmasjid.com/#svc-hallhire`.

   The redirect is a courtesy, not evidence. Anyone can visit that URL without
   paying, so the website never treats arriving there as proof of anything.
   Only this function marks a deposit paid.

## Proving it works

Stripe's dashboard has a **Send test webhook** button on the endpoint. Send a
`checkout.session.completed` and expect a `200`. Then in the SQL editor:

```sql
select action, detail, at from public.admin_audit
 where action in ('hall_deposit_paid','deposit_needs_refund',
                  'deposit_for_unknown_booking',
                  'nikah_fee_paid','nikah_fee_needs_refund',
                  'nikah_fee_for_unknown_request','nikah_fee_paid_twice',
                  'payment_without_reference','payment_with_unknown_reference_kind')
 order by at desc limit 10;
```

A test event carries no real `client_reference_id`, so expect
`payment_without_reference` — which proves the whole path: signature verified,
function ran, database written.

**Note the rename.** Until 7 September 2026 that line was written as
`deposit_without_reference`, when a hall deposit was the only thing anybody
could pay for. One endpoint now handles hall deposits and nikāḥ fees, so it is
`payment_without_reference`. Old rows keep the old name; a query looking for
history should ask for both.

For an end-to-end test, use Stripe **test mode** with card `4242 4242 4242
4242`, going through the booking form so a real reference is attached.

## The three things that go wrong

**A payment with no reference.** Somebody opened the Payment Link directly
rather than through the booking form. Logged as `payment_without_reference`
with their email. The office refunds it in Stripe.

**A payment for a reference that does not exist.** Logged as
`deposit_for_unknown_booking` for a hall, `nikah_fee_for_unknown_request` for
a nikāḥ. Same remedy.

**A reference in a shape this function does not recognise.** Logged as
`payment_with_unknown_reference_kind`. This means somebody has added a payment
link for something new — madrasah fees, say — without adding its prefix to the
`KINDS` table in `index.ts`. The money is safe and audited; the payment simply
is not recorded against anything.

**Two nikāḥ fees for one request.** Two family members both paying, or
somebody paying the member rate and then being told they are not a member.
The first payment is kept exactly as it was and the second is logged as
`nikah_fee_paid_twice`, naming both Stripe sessions. Nothing is overwritten —
the office refunds one of them.

**Two payments for one date.** The booking form holds a date for thirty
minutes, but a slow checkout can outlast that. The second payment is recorded
as `refund_due`, a `deposit_needs_refund` line is written, and the booking
appears in the venue portal marked for refund. Stripe already has the money;
somebody has to send it back.

All three are visible in the venue portal and in `admin_audit`. None of them
are silent.
